...
function groupEmojisByCategory(){
    emojiData.forEach(element => {
                          if(emojiCategories.indexOf(element.category) == -1){
                              emojiCategories.push(element.category)
                          }
                      });
    //console.log("emoji categories = "+JSON.stringify(emojiCategories))

    emojiCategories.forEach(emojiCategory =>{
                                let emojiModel = {}
                                emojiModel["category"] = emojiCategory
                                emojiModel["emojis"] = []

                                emojiData.forEach(element => {
                                                      if(element.category === emojiCategory){
                                                          let desc = element.description
                                                           if(element.tags.length > 0 ){
                                                              desc = desc.concat(" ", element.tags.join(" "))
                                                          }
                                                          emojiModel["emojis"].push({emoji: element.emoji, description: desc})
                                                      }
                                                  });
                                emojis.push(emojiModel)
                                // console.log("emoji model = "+JSON.stringify(emojiModel))
                            })

    console.log("emojis = "+JSON.stringify(emojis, 0, 4))
}
